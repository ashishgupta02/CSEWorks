...
import LZ.BinTree;
import LZMA.Base;
import RangeCoder.BitTreeEncoder;
...
public class Encoder {
  ...
  public static const(int) EMatchFinderTypeBT2 = 0;
  public static const(int) EMatchFinderTypeBT4 = 1;

  private static const(int) kIfinityPrice = 0xFFFFFFF;
  ...
  public static this() {
    ...
    for (int slotFast = 2; slotFast < kFastSlots; slotFast++) {
      ...
      for (int j = 0; j < k; j++, c++) {
        g_FastPos[c] = cast(byte)slotFast;
      }
    }
  }
  ...
  private static const(int) kDefaultDictionaryLogSize = 22;
  private static const(int) kNumFastBytesDefault = 0x20;
  ...
  class LiteralEncoder {
    ...
    class Encoder2 {
      ...
      // Done
      //-----------------------------------------------------------------------------------------------
      public int GetPrice(bool matchMode, byte matchByte, byte symbol) {
        int price = 0;
        int context = 1;
        int i = 7;
        if (matchMode) {
  ...
  public static const(int) kNumLenSpecSymbols = Base.kNumLowLenSymbols + Base.kNumMidLenSymbols;
  ...
  class LenPriceTableEncoder : LenEncoder {
    ...
    //
    //-----------------------------------------------------------------------------------------------
    override public void Encode(RangeEncoder rangeEncoder, int symbol, int posState) {
  ...
  private static const(int) kNumOpts = 1 << 12;
  ...
  // For Inner Class objects constructions should be done in non-static function
  // Modified code and now objects are constructed in constructor of Encoder
  ...
  private bool _needReleaseMFStream = false;
  ...
  private int ReadMatchDistances() {
    ...
    if (_numDistancePairs > 0) {
      ...
      if (lenRes == _numFastBytes) {
        lenRes += _matchFinder.GetMatchLen(cast(int)lenRes - 1, _matchDistances[_numDistancePairs - 1],
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  private int GetOptimum(int position) {
    ...
    while (true) {
      ...
      if (!nextIsChar && matchByte != currentByte) {
        // try Literal + rep0
        int t = min(numAvailableBytesFull - 1, _numFastBytes);
      ...
      for (int repIndex = 0; repIndex < Base.kNumRepDistances; repIndex++) {
        ...
        if (lenTest < numAvailableBytesFull) {
          int t = min(numAvailableBytesFull - 1 - lenTest, _numFastBytes);
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  private bool ChangePair(int smallDist, int bigDist) {
    int kDif = 7;
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public void CodeOneBlock(long[] inSize, long[] outSize, bool[] finished) {
    inSize[0] = 0;
    outSize[0] = 0;
    finished[0] = true;

    if (_inStream !is null) {
    ...
    if (nowPos64 == 0) {
      if (_matchFinder.GetNumAvailableBytes() == 0) {
        Flush(cast(int)nowPos64);
        return;
      }

      ReadMatchDistances();
      int posState = cast(int)(nowPos64) & _posStateMask;
      _rangeEncoder.Encode(_isMatch, (_state << Base.kNumPosStatesBitsMax) + posState, 0);
      _state = Base.StateUpdateChar(_state);
      byte curByte = _matchFinder.GetIndexByte(0 - _additionalOffset);
      _literalEncoder.GetSubCoder(cast(int)(nowPos64), _previousByte).Encode(_rangeEncoder, curByte);
    ...
    while (true) {
      int len = GetOptimum(cast(int)nowPos64);
      int pos = backRes;
      int posState = (cast(int)nowPos64) & _posStateMask;
      int complexState = (_state << Base.kNumPosStatesBitsMax) + posState;
      if (len == 1 && pos == -1) {
        _rangeEncoder.Encode(_isMatch, complexState, 0);
        byte curByte = _matchFinder.GetIndexByte(cast(int)(0 - _additionalOffset));
        LiteralEncoder.Encoder2 subCoder = _literalEncoder.GetSubCoder(cast(int)nowPos64, _previousByte);
        if (!Base.StateIsCharState(_state)) {
          byte matchByte = _matchFinder.GetIndexByte(cast(int)(0 - _repDistances[0] - 1 - _additionalOffset));
        ...
        if (pos < Base.kNumRepDistances) {
          ...
          if (posSlot >= Base.kStartPosModelIndex) {
            int footerBits = cast(int)((posSlot >> 1) - 1);
  ...
  private void ReleaseMFStream() {
    if (_matchFinder !is null && _needReleaseMFStream) {
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  private void SetStreams(InStream inStream, OutStream outStream,
  ...
  private bool[] finished = new bool[1];

  //-----------------------------------------------------------------------------------------------
  // Done
  //-----------------------------------------------------------------------------------------------
  public void Code(InStream inStream, OutStream outStream,
                   long inSize, long outSize) {
    _needReleaseMFStream = false;
    try {
  ...
  public static const(int) kPropSize = 5;
  private byte[] properties = new byte[kPropSize];

  //-----------------------------------------------------------------------------------------------
  // Done
  //-----------------------------------------------------------------------------------------------
  public void WriteCoderProperties(OutStream outStream) {
    properties[0] = cast(byte)((_posStateBits * 5 + _numLiteralPosStateBits) * 9 + _numLiteralContextBits);
    for (int i = 0; i < 4; i++) {
      properties[1 + i] = cast(byte)(_dictionarySize >> (8 * i));
    }
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public bool SetAlgorithm(int algorithm) {
    /*
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public bool SetNumFastBytes(int numFastBytes) {
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public bool SetMatchFinder(int matchFinderIndex) {
    if (matchFinderIndex < 0 || matchFinderIndex > 2) {
      return false;
    }
    int matchFinderIndexPrev = _matchFinderType;
    _matchFinderType = matchFinderIndex;
    if (_matchFinder !is null && matchFinderIndexPrev != _matchFinderType) {
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public void SetEndMarkerMode(bool endMarkerMode) {
