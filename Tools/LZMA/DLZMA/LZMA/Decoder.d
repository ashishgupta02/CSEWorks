...
import LZMA.Base;
import LZ.OutWindow;
import RangeCoder.BitTreeDecoder;
...
public class Decoder {
  ...
  class LenDecoder {
    ...
    // Done
    //-----------------------------------------------------------------------------------------------
    public int Decode(RangeDecoder rangeDecoder, int posState) {
  ...
  class LiteralDecoder {
    ...
    public void Create(int numPosBits, int numPrevBits) {
      if (m_Coders !is null && m_NumPrevBits == numPrevBits && m_NumPosBits == numPosBits) {
        return;
      }
  ...
  // Modified code and now objects are constructed in constructor of Decoder
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  private bool SetDictionarySize(int dictionarySize) {
    if (dictionarySize < 0) {
      return false;
    }
    if (m_DictionarySize != dictionarySize) {
      ...
      m_OutWindow.Create(max(m_DictionarySizeCheck, (1 << 12)));
    }
    return true;
  }
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  private bool SetLcLpPb(int lc, int lp, int pb) {
  ...
  // Done
  //-----------------------------------------------------------------------------------------------
  public bool Code(InStream inStream, OutStream outStream,
                      long outSize) {
    ...
    while (outSize < 0 || nowPos64 < outSize) {
      int posState = cast(int)nowPos64 & m_PosStateMask;

      if (m_RangeDecoder.DecodeBit(m_IsMatchDecoders, (state << Base.kNumPosStatesBitsMax) + posState) == 0) {
        LiteralDecoder.Decoder2 decoder2 = m_LiteralDecoder.GetDecoder(cast(int)(nowPos64), prevByte);
  ...
  public bool SetDecoderProperties(byte[] properties) {
    ...
    for (int i = 0; i < 4; i++) {
      dictionarySize += (cast(int)(properties[1 + i]) & 0xFF) << (i * 8);
    }
